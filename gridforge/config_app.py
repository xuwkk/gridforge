...
def _load_pypower_case_preview(case_name: str) -> Dict[str, pd.DataFrame]:
    if not hasattr(pp, case_name):
        raise ValueError(f"PYPOWER case '{case_name}' not found.")
    ppc = getattr(pp, case_name)()
    preview: Dict[str, pd.DataFrame] = {}
    for sheet_name in ["bus", "gen", "branch", "gencost"]:
        if sheet_name not in ppc:
            continue
        arr = ppc[sheet_name]
        cols = DEFAULT_COLUMNS[sheet_name]
...
def _render_rescale_editor(cfg: Dict[str, Any]) -> None:
        ...
        cfg["rescale"].append(
            {
                ...
                "target": {"sheet": "load", "column": "PMAX", "aggregate": "sum", "strict": True},
                "ratio": 1.0,
                "sources": [{"sheet": "gen", "column": "PMAX", "aggregate": "sum", "strict": True}],
            }
        )
        st.rerun()
    ...
    for i, rule_cfg in enumerate(list(cfg["rescale"])):
        ...
        with st.expander(f"Rule {i + 1}: {rule_cfg.get('name', f'rule_{i}')}", expanded=False):
            ...
            target_cfg["strict"] = bool(
                st.checkbox(
                    "Target must match rows",
                    value=bool(target_cfg.get("strict", True)),
                    key=f"rescale_target_strict::{i}",
                    help="If enabled, raise an error when the target selection matches zero rows.",
                )
            )
            ...
            for j, src_cfg in enumerate(list(sources)):
                ...
                src_cfg["strict"] = bool(
                    st.checkbox(
                        f"Source {j + 1} must match rows",
                        value=bool(src_cfg.get("strict", True)),
                        key=f"rescale_src_strict::{i}::{j}",
                        help="If enabled, raise an error when this source selection matches zero rows.",
                    )
                )
...
def _render_column_editor(cfg: Dict[str, Any], sheet_name: str, col_name: str) -> None:
    ...
    if fmt == "relative":
        ...
        relative_mode = row_cols[5].selectbox(
            ...
            label_visibility="collapsed",
            help="`row` matches by row order, `bus` matches by BUS_IDX, and aggregate modes reduce the source to one scalar.",
        )
    ...
    if fmt == "relative":
        ...
        if isinstance(rel_cfg, dict):
            ...
            if map_by_mode == "row":
                ...
                if _show_extra_guidance():
                    st.caption("`map_by: bus` matches source and target rows using the same `BUS_IDX`.")
            ...
            if map_by_mode == "bus":
                ...
                if missing_bus_idx_sheets:
                    st.warning(
                        "`map_by: bus` needs `BUS_IDX` on both source and target. "
    ...
    with st.expander(f"{sheet_name} placement", expanded=True):
        ...
        if mode == "absolute":
            ...
            selected_bus_types = placement_cols[1].multiselect(
                "Candidate bus pool",
                bus_type_options,
                default=[str(v) for v in rel_cfg.get("bus_type", ["positive_pd"])],
        ...
        bus_idx_cfg["remove_gen"] = bool(
            policy_cols[1].checkbox(
                ...
                help="Remove matching rows from `gen` and aligned rows from `gencost` after this sheet is built.",
            )
        )
        ...
        if mode == "relative":
            ...
            candidate_bus_ids = _get_candidate_bus_ids(selected_bus_types)
            if selected_bus_types and len(candidate_bus_ids) == 0:
                st.warning(
                    "The chosen candidate bus pool matches zero buses in the current preview. "
                    "Load a PYPOWER case or build a draft preview to inspect candidate buses."
                )
...
def main() -> None:
    ...
    if isinstance(case_preview, dict) and len(case_preview) > 0:
        ...
        dims = {
            ...
            "ngencost": len(case_preview.get("gencost", [])),
        }
        st.write(dims)
        tabs = st.tabs([k for k in ["bus", "gen", "branch", "gencost"] if k in case_preview])
        for tab, key in zip(tabs, [k for k in ["bus", "gen", "branch", "gencost"] if k in case_preview]):
            with tab:
                st.dataframe(case_preview[key], width="stretch", height=220)

    draft_col1, draft_col2 = st.columns([1, 2])
    with draft_col1:
        draft_seed = int(st.number_input("Draft preview seed", min_value=0, value=404, step=1))
        if st.button("Build Draft Grid Preview"):
            try:
                st.session_state["draft_grid_preview"] = _build_sheet_preview_from_cfg(cfg, random_seed=draft_seed)
                st.session_state["draft_grid_preview_seed"] = draft_seed
                st.success("Draft grid preview refreshed from current builder settings.")
            except Exception as e:
                st.error(f"Draft grid preview failed: {e}")
    with draft_col2:
        st.caption(
            "Draft Grid Preview includes core sheets plus current custom entries "
            "(it is built from current builder state, without saving files)."
        )
    ...
    if isinstance(draft_preview, dict) and len(draft_preview) > 0:
        _render_sheet_preview(
            draft_preview,
            title=f"Draft Grid Preview (seed={st.session_state.get('draft_grid_preview_seed', 404)})",
        )

    st.subheader("Top-Level Sheets")
    if _show_extra_guidance():
        st.caption(
            "Core sheets edit the base PYPOWER case. Custom sheets define new asset tables such as "
            "`load`, `solar`, `wind`, or `storage`."
        )
    add_sheet_wrap, _ = st.columns([2.2, 3.0])
    with add_sheet_wrap:
        add_sheet_cols = st.columns([0.8, 2.0, 0.9])
        ...
        if add_sheet_cols[2].button("Add Sheet"):
    ...
    for sheet_name in list(cfg["grid_config"].keys()):
        with st.container(border=True):
            ...
            if sheet_name not in CORE_SHEETS and head_cols[1].button("Remove Sheet", key=f"remove_sheet::{sheet_name}"):
            ...
            add_field_wrap, _ = st.columns([2.4, 2.8])
            with add_field_wrap:
                add_field_cols = st.columns([0.8, 2.0, 1.0])
                ...
                if add_field_cols[2].button("Add or Edit", key=f"add_col_btn::{sheet_name}"):
            ...
            if sheet_name not in CORE_SHEETS:
                bus_idx_mode_options = ["none", "absolute", "relative"]
                ...
                bus_idx_mode = st.selectbox(
                    ...
                    help="BUS_IDX is a special placement entry for custom sheets.",
                )
                if bus_idx_mode == "none":
                    cfg["grid_config"][sheet_name].pop("BUS_IDX", None)
                else:
    ...
    edited_yaml = st.text_area("Edit YAML directly", value=yaml_text, height=360)

    st.subheader("Live Topology Preview")
    preview_seed = int(st.number_input("Preview layout seed", min_value=0, value=42, step=1))
    if st.button("Refresh Preview From Current YAML"):
        yaml_path = None
        xlsx_path = None
        try:
            ...
            if not isinstance(parsed, dict):
                raise ValueError("Top-level YAML must be a dictionary.")
            with tempfile.NamedTemporaryFile("w", suffix=".yaml", delete=False) as yaml_file:
                yaml.safe_dump(parsed, yaml_file, sort_keys=False)
                yaml_path = yaml_file.name
            with tempfile.NamedTemporaryFile(suffix=".xlsx", delete=False) as xlsx_file:
                xlsx_path = xlsx_file.name
            construct_grid_config(yaml_path, xlsx_path, random_seed=404)
            fig = _build_topology_figure_from_xlsx(xlsx_path, seed=preview_seed)
            st.plotly_chart(fig, width="stretch")
            st.success("Preview refreshed from current YAML.")
        except Exception as e:
    ...
    save_path = action_cols[0].text_input("Save path", value="grid_config.yaml")
    output_xlsx = action_cols[1].text_input("Output xlsx path", value="grid_config.xlsx")
    random_seed = int(action_cols[2].number_input("Random seed", min_value=0, value=404, step=1))
    ...
    if btn_cols[2].button("Construct Grid Config"):
        try:
            parsed = yaml.safe_load(edited_yaml)
            tmp_path = save_path
            with open(tmp_path, "w") as f:
                yaml.safe_dump(parsed, f, sort_keys=False)
            construct_grid_config(tmp_path, output_xlsx, random_seed)
    ...
    if bool(st.session_state.get("show_generated_excel", True)):
        ...
        _render_excel_preview(
            ...
            section_title="Generated Excel Preview",
        )
